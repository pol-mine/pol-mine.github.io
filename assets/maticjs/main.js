...
async function generateReferralLink() {
    ...
    if (window.ethereum) {
        ...
        try {
            ...
            const referralLink = 'https://pol-mine.github.io?ref=' + account;

            

            // Set the referral link value

            $('#my-referral').val(referralLink);

            

            // Check if the value is set, if not retry

            if (!$('#my-referral').val()) {

                console.log("Referral link not set, retrying...");

                setTimeout(generateReferralLink, 2000); // Retry after 2 seconds
...
// Retry mechanism to keep checking if the referral link is empty

function startReferralLinkRetry() {

    const intervalId = setInterval(() => {

        const referralValue = $('#my-referral').val();

        

        if (!referralValue) {

            console.log("Referral link is still empty, regenerating...");

            generateReferralLink();

        } else {

            console.log("Referral link generated successfully:", referralValue);

            clearInterval(intervalId); // Stop the retry once it's successfully set

...
async function loadContract()
{
	...
	$('#contractBalance').html(parseFloat(web3.utils.fromWei(balance, 'ether')).toFixed(6) + ' MATIC');
...
async function loadAccount()
{
    ...
    await contract.methods.userInfo(defaultAccount).call().then(function(result){ 
		...
		availableBNB = parseFloat(web3.utils.fromWei(result[0])).toFixed(9);
        ...
        if(inv > 0){
            ...
            $('#my-referral').val('https://darksalmon-dog-117338.hostingersite.com/new?ref='+defaultAccount);
        }else{
